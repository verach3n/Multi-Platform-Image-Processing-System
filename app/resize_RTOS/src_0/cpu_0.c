...
#define TASK1_PRIORITY      16
#define TASK2_PRIORITY      15
#define TASK3_PRIORITY      17
...
void task1(void* pdata)
{
	...
	{
		//printf("task 1 start!\n");
		resizedata = malloc(QUEUE_SIZE / 4 *sizeof(unsigned char));
		msg = OSQPend(CommQ, 0, &err);
		gray = (unsigned char*) msg;
		resize(gray, resizedata, 32, 32);
		err = OSQPost(CommQ, (void *)resizedata);
		free(resizedata);
...
void task3(void* pdata)
{
	...
	{ 
		//printf("task 3 start!\n");
		msg = OSQPend(CommQ, 0, &err);
		processedimage = (unsigned char*) msg;
		//printf("pixel2: %f, %f, %f\n", (float)gray[0],(float)gray[1],(float)gray[2]);
		printAscii(processedimage, 16, 16);
